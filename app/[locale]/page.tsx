...
export default function Home({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      <motion.section initial={{y:30, opacity:0}} animate={{y:0, opacity:1}} transition={{delay:0.1, duration:0.6}} className="bg-gradient-to-r from-white to-gray-50">
          <div className="max-w-6xl mx-auto px-4 py-12 grid md:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <motion.div key="home-contact-link" whileHover={{scale:1.02}} whileTap={{ scale: 0.95 }} transition={{ type: 'spring', stiffness: 300 }} className="inline-block">
                <Link href={`/${locale}/contact`} className="inline-block px-6 py-3 bg-primary text-white rounded">{t.hero.cta}</Link>
              </motion.div>
            </div>
            <motion.div key={pathname + '-contact-btn'} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.4}} className="rounded-lg overflow-hidden shadow-lg">
              <img src="https://picsum.photos/800/520?random=1" alt="hero" className="w-full h-64 md:h-80 object-cover" />
            </motion.div>
          </div>
      </motion.section>

      <motion.section initial={{y:30, opacity:0}} animate={{y:0, opacity:1}} transition={{delay:0.1, duration:0.6}} className="max-w-6xl mx-auto px-4 py-12">
